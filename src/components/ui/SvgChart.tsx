...
const STATUS_CONFIG: Record<ToothStatus, { label: string; fill: string; border: string }> = {
    // Sănătos rămâne alb solid ca să se vadă curat
    HEALTHY: { label: 'Sănătos', fill: '#ffffff', border: '#0f172a' },
    
    // Culori cu transparență 50% (0.5)
    ...
    // Lipsă rămâne complet transparent
    MISSING: { label: 'Lipsă', fill: 'transparent', border: '#6b7280' }
  };
// Coordonatele X aproximative pentru centrul fiecărui dinte (pe partea dreaptă)
...
export default function SinglePatientDentalChart({ patientId, patientName }: SinglePatientDentalChartProps) {
    ...
    const loadToothData = async (id: string) => {
      ...
      try {
        // Simulare load - înlocuiește cu API-ul tău real
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-8 border border-gray-200 dark:border-gray-700">
          <svg viewBox="0 60 220 180" className="w-full h-auto max-w-5xl mx-auto">
            
            {/* ================= GINGIA (Background) ================= */}
            {/* Este pusă prima pentru a fi "în spate". Are pointerEvents: none pentru a da click prin ea */}
            <g id="gums-layer" style={{ pointerEvents: 'none'  }} >
            {/* Desenăm gingia o singură dată, așa cum ai pus-o tu */}
            <path 
              d={GUM_PATH_UPPER} 
              fill="#fbeff3" 
              stroke="none" 
              ...
              {TEETH_UPPER.map((tooth) => {
                ...
                return (
                  <path
                    ...
                    style={{
                      filter: selectedTooth === tooth.id ? 'drop-shadow(0 0 3px rgba(59, 130, 246, 0.8))' : 'none'
                    }}
                  />
                );
              })}
            ...
            {/* ARCADA SUS STANGA (Cadranul 2 - Oglindă) */}
              ...
              {TEETH_UPPER.map((tooth) => {
                ...
                return (
                  <path
                    ...
                    strokeWidth="0.12"
                    className="cursor-pointer transition-all hover:opacity-70"
                    onClick={() => setSelectedTooth(leftId)}
                    style={{
                      filter: selectedTooth === leftId ? 'drop-shadow(0 0 3px rgba(59, 130, 246, 0.8))' : 'none'
                    }}
                  />
                );
              })}
            ...
            {/* ARCADA JOS STANGA (Cadranul 3 - Oglindă) */}
  ...
  {/* --- ARCADA SUS (MAXILAR) --- */}
  {TEETH_UPPER.map((tooth) => {
    // Coordonatele X pentru partea dreaptă (Q1: 11-18)
    const xRight = TOOTH_CENTERS[tooth.id] || 0;
    // Calculăm simetria pentru stânga (Q2: 21-28) - Ajustat pentru gap-ul tău
    const xLeft = 216.5 - xRight-5; 

    return (
      <React.Fragment key={`label-upper-${tooth.id}`}>
        {/* Dreapta (Ex: 18) - Coborât puțin la y=65 */}
  ...
  {TEETH_LOWER.map((tooth) => {
    ...
    return (
      <React.Fragment key={`label-lower-${tooth.id}`}>
        {/* Dreapta (Ex: 48) - Coborât mult sub dinți la y=175 */}
                ...
                <button
                  ...
                  className={`p-3 rounded-lg border-2 transition-all ${
                    selectedToothData?.status === status
                      ? 'border-blue-500 dark:border-blue-400 bg-blue-50 dark:bg-blue-900/30 ring-2 ring-blue-200'
